...
export default function VISAPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const url = editingData ? `/api/visa/${editingData.id}` : "/api/visa";
  ...
  const getStatusColor = (status: string) => {
    const colors: Record<string, string> = {
      ...
      "Upload Dokumen": "bg-yellow-900/30 text-yellow-400 border border-yellow-700/50",
      "Apply VISA": "bg-blue-900/30 text-blue-400 border border-blue-700/50",
      "Proses oleh Embassy": "bg-purple-900/30 text-purple-400 border border-purple-700/50",
    };
  ...
  const filteredData = data.filter((item) => {
    const matchSearch = 
      ...
      item.negaraTujuan?.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="min-h-screen bg-[#0a0a0a]">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-lg font-semibold text-white hidden sm:block">VISA</h2>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div>
            <h2 className="text-2xl font-bold text-[#D4AF37]">Daftar VISA</h2>
            <p className="text-sm text-gray-400">Kelola dokumen VISA pelaut</p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <input
            type="text"
            placeholder="Cari nama, kode pelaut, atau negara..."
            value={searchTerm}
          ...
          >
            ...
            <option value="Apply VISA">Apply VISA</option>
            <option value="Proses oleh Embassy">Proses oleh Embassy</option>
            ...
            <table className="w-full">
              <thead className="bg-[#0a0a0a] border-b border-[#D4AF37]/10">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-[#D4AF37] uppercase tracking-wider">Kode Passpor</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-[#D4AF37] uppercase tracking-wider">Negara Tujuan</th>
                ...
                {filteredData.length === 0 ? (
                  ...
                  filteredData.map((item) => (
                    <tr key={item.id} className="hover:bg-[#0a0a0a] transition-colors">
                      ...
                      <td className="px-6 py-4 font-mono text-sm text-gray-400">{item.kodePasspor}</td>
                      <td className="px-6 py-4">
                        <span className="px-3 py-1 rounded-full text-xs font-medium bg-indigo-900/30 text-indigo-400 border border-indigo-700/50">
                          {item.negaraTujuan}
                        </span>
                      </td>
                      <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-[#1a1a1a] border border-[#D4AF37]/30 rounded-xl shadow-2xl w-full max-w-md p-6">
            <h3 className="text-xl font-bold text-[#D4AF37] mb-4">
              {editingData ? "Edit VISA" : "Tambah VISA"}
            </h3>
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-[#D4AF37] mb-1">Kode Passpor</label>
                  <input
                    type="text"
                    value={formData.kodePasspor}
                    onChange={(e) => setFormData({ ...formData, kodePasspor: e.target.value })}
                    className="w-full px-4 py-2 bg-[#0a0a0a] border border-[#D4AF37]/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#D4AF37] text-white"
                    required
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-[#D4AF37] mb-1">Negara Tujuan</label>
                  <input
                    type="text"
                    ...
                    className="w-full px-4 py-2 bg-[#0a0a0a] border border-[#D4AF37]/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#D4AF37] text-white"
                    required
                    placeholder="Contoh: Schengen, USA, UK"
                  />
                </div>
                <div>
