...
export default function ESIDPage() {
  const { data: session, status } = useSession();
  const [data, setData] = useState<ESID[]>([]);
  ...
  const fetchData = async () => {
    try {
      const res = await fetch("/api/e-sid");
      if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const url = editingData ? `/api/e-sid/${editingData.id}` : "/api/e-sid";
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Yakin ingin menghapus data ini?")) {
      try {
        const res = await fetch(`/api/e-sid/${id}`, { method: "DELETE" });
        if (res.ok) {
          fetchData();
        }
      } catch (error) {
  ...
  const getStatusColor = (status: string) => {
    const colors: Record<string, string> = {
      ...
      Approval: "bg-blue-900/30 text-blue-400 border border-blue-700/50",
      "Tanggal Foto": "bg-purple-900/30 text-purple-400 border border-purple-700/50",
    };
    ...
    <div className="min-h-screen bg-[#0a0a0a]">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-lg font-semibold text-white hidden sm:block">E-SID</h2>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div>
            <h2 className="text-2xl font-bold text-[#D4AF37]">Daftar E-SID</h2>
            <p className="text-sm text-gray-400">Kelola dokumen E-SID pelaut</p>
          </div>
            ...
            <table className="w-full">
              <thead className="bg-[#0a0a0a] border-b border-[#D4AF37]/10">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-[#D4AF37] uppercase tracking-wider">Lokasi</th>
                ...
                {filteredData.length === 0 ? (
                  ...
                  filteredData.map((item) => (
                    <tr key={item.id} className="hover:bg-[#0a0a0a] transition-colors">
                      ...
                      <td className="px-6 py-4 text-gray-400">{item.lokasiPembuatan}</td>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-[#1a1a1a] border border-[#D4AF37]/30 rounded-xl shadow-2xl w-full max-w-md p-6">
            <h3 className="text-xl font-bold text-[#D4AF37] mb-4">
              {editingData ? "Edit E-SID" : "Tambah E-SID"}
            </h3>
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-[#D4AF37] mb-1">Lokasi Pembuatan</label>
                  <select
                    value={formData.lokasiPembuatan}
                    onChange={(e) => setFormData({ ...formData, lokasiPembuatan: e.target.value })}
                    className="w-full px-4 py-2 bg-[#0a0a0a] border border-[#D4AF37]/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#D4AF37] text-white"
                  >
